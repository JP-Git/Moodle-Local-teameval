...
class team_evaluation {
    ...
    ******************/
    ...
    public function did_submit_response($type, $id, $userid) {
        //TODO: isn't there a cache of the questiontype->questionid->question_info somewhere?
        foreach($this->get_questions() as $qi) {
            if (($qi->type == $type) && ($qi->questionid == $id)) {
                unset(self::$response_cache[$qi->id][$userid]);
                break;
            }
        }
    ...
    protected function score_to_multiplier($score, $uid) {
        if (is_null($score)) {
            // If the team hasn't completed evaluation, everyone gets 1.0
            $multiplier = 1.0;
        } else {
    ...
    /**
     * Check that the groups are in correct working order for TE.
     * Every user must belong to exactly one group, and every group must have at least two members.
     * @return string|null The failure reason if the check failed, otherwise null
     */
    public function sanity_check_team_members() {
        $reverse_members = [];
        $group_members = [];
        foreach ($this->get_evaluation_context()->marking_users('u.*') as $userid => $user) {

            $group = $this->get_evaluation_context()->group_for_user($userid);
            if (empty($group)) {
                continue;
            }
            $groupid = $group->id;

            $reverse_members[$userid] = $groupid;
            if (!isset($group_members[$groupid])) {
                $group_members[$groupid] = [];
            }
            $group_members[$groupid][$userid] = $user;
        }

        $all_groups = $this->get_evaluation_context()->all_groups();

        $_group_name = function ($groupid) {
            if (isset($all_groups[$groupid])) {
                return $all_groups[$groupid]->name;
            } else if (empty($groupid)) {
                return get_string('nogroup', 'group');
            } else {
                $group = groups_get_group($groupid, 'id,name');
                if ($group) {
                    return $group->name;
                } else {
                    return "[id=$groupid]";
        ...
        // Now check that the groups match all_groups
        // This checks that there are no groups returned by group_for_user() that weren't in all_groups()
        $extra_groups = array_diff_key($group_members, $all_groups);
        if (!empty($extra_groups)) {
            $extragroupstr = implode(', ', array_map($_group_name, array_keys($extra_groups)));
            return get_string('extragroups', 'local_teameval', $extragroupstr);
        }

        // This checks that there are no groups returned by all_groups() that have no corresponding group_for_user()
        $missing_groups = array_diff_key($all_groups, $group_members);
        if (!empty($missing_groups)) {
            $missinggroupstr = implode(', ', array_map($_group_name, array_keys($missing_groups)));
            return get_string('missinggroups', 'local_teameval', $missinggroupstr);
        }

        // Now check the membership is as expected. If there's an issue here, the problem is probably users in multiple groups.
        foreach ($all_groups as $groupid => $group) {
            $members = $this->group_members($groupid);

            // Check if group_for_user returned too many people
            $extra_members = array_diff_key($group_members[$groupid], $members);
            if (!empty($extra_members)) {
                $extras = implode(array_map('fullname', $extra_members));
                return get_string('extramembers', 'local_teameval', ['group' => $group->name, 'extras' => $extras]);
            }

            // Check if group_members returned too many people
            $missing_members = array_diff_key($members, $group_members[$groupid]);
            if (!empty($missing_members)) {
                $missings = implode(array_map('fullname', $missing_members));
                return get_string('missingmembers', 'local_teameval', ['group' => $group->name, 'missing' => $missings]);
            }
        }

        // Check that all groups have at least two members
        $toofewusers = array_filter($group_members, function($v) { return count($v) < 2; });
        if (!empty($toofewusers)) {
            $toofewusers = array_map($_group_name, array_keys($toofewusers));
            $toofewuserstr = implode(', ', $toofewusers);

            return get_string('nosingleusergroups', 'local_teameval', $toofewuserstr);
        }
    ...
    public function teammates($userid, $include_self=null) {
        ...
        // Guard that the user is actually in the returned group
        if (!array_key_exists($userid, $members)) {
            throw new moodle_exception('notinowngroup', 'local_teameval', '', $userid);
        }
    ...
    /**
     * Get all feedback strings given to a user
     * @param  int     $userid    the user who we're collecting feedback for
     * @param  boolean $rescinded include rescinded feedback items
     * @return array   the feedback objects grouped by question. Objects look like:
     *                 ->question: question_info, ->teammates: [ ->from: user record, ->feedback: response ]
     */
    public function all_feedback($userid, $rescinded=true) {
        ...
        foreach($questions as $qi) {
            ...
            $teammates = $this->teammates($userid);

            $this->prepare_all_responses($teammates);
            ...
            foreach($teammates as $tm) {
                if (!$rescinded && ($this->rescinded($qi->id, $tm->id, $userid) == \local_teameval\FEEDBACK_RESCINDED)) {
                    continue;
                }
